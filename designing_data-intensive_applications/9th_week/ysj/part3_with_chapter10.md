...
- 시스템의 3가지 유형
    ...
    - 스트링 처리 시스템(준실시간 시스템)
        ...
        - 입력 데이터가 한정되지 않는다. 끝이 없는 데이터 스트림이다.
...
- 일괄 처리 워크플로의 출력
    ...
    - 검색 색인 구축:
        - 구글이 처음 만든 이유가 검색 색인 구축 목적이였음. (지금은 사용하지 않는다고 함)
        - 지금도 검색 색인 구축으로서 유용하다.
        - 정해진 문서 집합을 대상으로 전문 검색이 필요하다면 일괄 처리가 매우 효율적이다.
    - 일괄 처리의 출력으로 키-값을 저장:
        - 분류기 같은 머신러닝 or 추천 시스템 구축 등에서 유용하다.
        - 일괄 처리의 결과로 데이터베이스 파일을 생성한다.
        - 일괄 처리의 결과는 read-only이므로 매우 단순하게 구축할 수 있다. (WAL이 필요없다.)
            - (처리 전/동안에는 이전 데이터 보여주고, 처리 완료되면 새 데이터 보여주고...)
    - 일괄처리 출력에 관한 철학:
        - 유닉스 철학과 같이 입/출력을 철학으로 취급한다. (물론 하둡이 유닉스랑 완전히 같은 설계 원칙을 가진다는 말은 아님. 비슷하긴 하지만)
        - 입/출력 모두 불변으로 처리하고 부수 효과를 피하기 때문에, 좋은 성능을 내면서도 유지보수가 간단하다.
            - 실패 회복 쉽게 가능, 입/출력과 로직의 분리 등

#### 하둡과 분산 데이터베이스의 비교

- 개요
    - HDFS는 파일 시스템이고, 맵리듀스는 특별한 방식으로 구현된 유닉스 프로세스다.
    - 맵리듀스의 처리 알고리즘과 병렬 조인 기법은 대규모 병렬 처리(MPP) 데이터베이스에서 이미 상당수 구현되었다.
    - 단, 맵리듀스는 MPP와 달리 높은 다형성을 제공한다.
- 하둡의 다형성
    - 저장소의 다형셩
        - 데이터가 어떤 형태라도 HDFS로 덤프할 수 있는 가능성을 열어 놓았다.
        - 데이터의 형식이 일정하지 않더라도 (원시) 사용할 수 있다.
    - 처리 모델의 다형셩    
        - 접근법이 다른 여러 처리 모델(HBase, 임팔라 등)에서 HDFS를 사용한다.
        - MPP와 달리 SQL 말고도 여러 표현법을 사용할 수 있다.
    - 결함을 줄이는 설계
        - 맵리듀스는 올라인 시스템에 비해 결함에 덜 민감하다. 사용자가에 영향을 주지 않으면서 재시도 가능하기 때문이다.
        - MPP는 실패를 견디지 못한다. 한 장비만 죽어도 전체 질의가 중단된다.
        - 맵리듀스는 개별 테스크 단위로 수행하므로 일부 맵/리듀스 태스트의 실패를 견딜 수 있다.
        - 맵리듀스는자주 실패하고 다시 실행할 수 있게 설계되었다.
            - 자원 할당 우선순위가 낮다.
            - 초창기 구글에서 혼합 사용 데이터 센터를 사용했고, 우선순위 높은 태스크가 많은 경우 실패시키고 다시 실행해야 했기 때문이다.

### 맵리듀스를 넘어

- 맵리듀스는 분산 가능한 여러 프로그래밍 모델 중 하나다. 자료구조, 데이터 량, 처리 방식에 따라 다른 도구가 유용할 수도 있다.

- 중간 상태 구체화: 맵 리듀스는 워크플로의 중간 상태를 무조건 파일로 저장하고, 이에 따라 여러 단점이 있다.
    - 데이터 플로 엔진: 
        - 전체 워크플로를 독립된 하위 작업으로 나누지 않고 작업 하나로서 다룬다. 
        - 맵,리듀스의 반복 없이 더 자유로운 함수 조합이 가능하다.
        - 필요할 때만 저장한다.
    - 내결함성: 
        - 중간 상태를 쓰지 않고 내결함성을 얻기 위해, 실패하면 유효한 데이터부터 재시도 방법을 사용한다.
        - 그러나 중간 데이터가 적거나, CPU 중심적이라면 재연산보다 중간 결과를 저장하는게 더 효과적이다.
- 그래프와 반복 처리
    - 맵리듀스는 데이터를 일회성으로 처리하는데, 이는 그래프 알고리즘의 특징과 잘 맞지 않는다.
        - 그래프 알고리즘은 간선을 통해서 반복 수행되는 반복적 스타일로 구현된다.
    - 일괄 처리 그래프를 최적화 하는 방법으로 벌크 동기식 병렬(BSP) 연산 모델이 널리 사용된다.
    - (생략 (내결함성, 병렬 실행)) - 정리할 정도는 아닌듯?
- 고수준 API와 언어
    - 맵리듀스 작업을 직접 작성하는 일은 어렵기 때문에 이를 도와주는 고수준 API나 언어가 인기를 끈다.
        - 사용자가 생산성 있게, 효율적으로 작업할 수 있도록 해준다.
    - 선언적 질의 언어로 전환 (QL)
    - 다양한 분야를 지원하기 위한 전문화

## 느낀점

유닉스 시스템의 파이프라인 설명은 인상깊었다. 생각해보지도 못했던 주제였음.    
하긴 CLI라서 프로그래밍 언어에서 제공하는 기능이라고 생각했는데, 그것들도 다 프로그램이라 구현한 사람도 다를꺼고... 그런데도 잘 동작한다.    


하둡이나 맵 리듀스를 써본적도 없고 접하기 어렵다보니 이해가 잘 안됨.     
그냥 외우듯이 읽고 있는데, 별로 좋은 상태는 아니라 생각함.   

[\[NHN FORWARD 22\] Hadoop 개발 환경 구축과 MySQL 기반 배치를 Hadoop으로 옮긴 이야기](https://www.youtube.com/watch?v=-5Kul72jqLQ) 해당 영상에서 어떤 식으로 사용하는지 알 수 있었음. (중간단계 구축 과정은 스킵하고 봄)   
특히 이전 장들에서 나온 내용이 나와서 더 흥미롭게 읽을 수 있었음.    
    (전체 순서 브로드캐스트 프로토콜을 지원하는)주키퍼나 리더 노드 산출을 위해서 과반수가 필요하므로 최소 노드가 3개라는 내용.

이건 사담인데, 다음 스터디를 한다면 좀 더 실용적인? 주제를 잡으면 좋겠음.   
이 책도 좋은 책이지만, 취준생/주니어 입장에서 지금 당장 적용/응용 가능한 실용적인 주제는 많이 없다.   